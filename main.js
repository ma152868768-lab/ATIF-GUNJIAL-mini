...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [Atif-Gunjial-MD] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'ATIF-GUNJIAL-MD' };
            }
        });
            ...
            if (connection === 'open') {
                ...
                if (!existingSession) {
                    await conn.sendMessage(userJid, {
                        ...
                        caption: `\n╭────────────────────◇\n│✦ *𝗔𝗧𝗜𝗙-𝗚𝗨𝗡𝗝𝗜𝗔𝗟-𝗠𝗗 — CONNECTED* 🔥\n│✦ Type *${prefix}menu* to see all commands 💫\n│✦ Prefix 『 ${prefix} 』  Mode 〔${mode}〕\n╰────────────────────○\n*© Powered by 𝗧𝗘𝗔𝗠-𝗦𝗛𝗔𝗗𝗢𝗪*`
                    });
                }
            }
            if (connection === 'close') {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363427946700389@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© 𝗔𝗧𝗜𝗙-𝗚𝗨𝗡𝗝𝗜𝗔𝗟-𝗠𝗗',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:ATIF-GUNJIAL-MD BOY\nORG:ATIF GUNJIAL-MD BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
        ...
        arslanLog(`ATIF GUNJIAL-MD Pair error: ${err.message}`, 'error');
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'Atif Gunjial-md is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 ATIF GUNJIAL-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
